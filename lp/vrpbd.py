...
# 22-23
for k in K:
    model += p[k, 0] == pl.lpSum([q[u] * x[k, u] for u in L]) + pl.lpSum([q[u] * x_tilde[k, u] for u in L for r in R])
    model += p[k, end_depot_idx] == -pl.lpSum([q[u] * x[k, u] for u in B]) - pl.lpSum([q[u] * x_tilde[k, u] for u in B for r in R])
